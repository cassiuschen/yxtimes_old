  ...
  after_create do
    ...
    if self.commentable.is_a?(Article)
      ...
      self.commentable.starrers.each do |user|
        ...
        user.send_notification("#{commenter ? commenter.name : "匿名用户"} 评论了文章《#{commentable.title}》", article_path(commentable))
      end
      ...
      unless self.commentable.starrers.include? self.commentable.author or self.commenter==self.commentable.author
        self.commentable.author.send_notification("#{commenter ? commenter.name : "匿名用户"} 评论了文章《#{commentable.title}》", article_path(commentable))
      end
